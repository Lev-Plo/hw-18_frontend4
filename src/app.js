import { getStudents } from "./js/get-students";
import { deleteStudents } from "./js/remove-students";
import { addStudent } from "./js/add-students";
import { updateStudents } from "./js/update-students";
...
function createItems(array) {
    const elem = array.map(({id,name,age,course,skills,email,isRegistered})=>{
        return `<tr id="${id}">
        <td>${id}</td>
    ...
    <td>${isRegistered?"Випустився":"Невипустився"}</td>
    <td>
...
formRef.addEventListener("submit",async (e)=>{
    ...
    const data = {
        ...
        isRegistered:e.currentTarget.elements.isRegistered.checked,
    }
    if (currentID) {
        try {
